 ...
 * NOTE: Only used with EXPLICIT buffer strategy, not needed for INFINITE.
 *
 * param provider ImageProvider pointer.
...
// Allocate VDO buffers for EXPLICIT strategy
static bool allocateVdoBuffers(ImgProvider_t* provider, VdoStream* vdoStream) {
    GError* error = NULL;
    bool ret      = false;
...
bool createStream(ImgProvider_t* provider, unsigned int w, unsigned int h) {
    ...
    const char* format_name = (provider->vdoFormat == VDO_FORMAT_YUV) ? "YUV" :
                              (provider->vdoFormat == VDO_FORMAT_JPEG) ? "JPEG" : "UNKNOWN";
    syslog(LOG_INFO, "%s: Creating VDO stream with resolution=%ux%u, format=%s, channel=%d, strategy=INFINITE",
           __func__, w, h, format_name, VDO_CHANNEL);
    ...
    // Use INFINITE strategy (default) - VDO manages buffers
    vdo_map_set_uint32(vdoMap, "buffer.strategy", VDO_BUFFER_STRATEGY_INFINITE);
    vdo_map_set_uint32(vdoMap, "buffer.count", NUM_VDO_BUFFERS);
    ...
    if (!vdoStream) {
        syslog(LOG_ERR,
               "%s: Failed creating vdo stream: %s (error_code=%d, error_domain=%s)",
               __func__,
               (error != NULL) ? error->message : "N/A",
               (error != NULL) ? error->code : -1,
               (error != NULL) ? g_quark_to_string(error->domain) : "N/A");
        goto errorExit;
    }
    syslog(LOG_INFO, "%s: VDO stream created successfully", __func__);

    // INFINITE strategy - VDO manages buffers, no manual allocation needed

    // Start the actual VDO streaming.
    syslog(LOG_INFO, "%s: Starting VDO stream...", __func__);
    if (!vdo_stream_start(vdoStream, &error)) {
        syslog(LOG_ERR,
               "%s: Failed starting stream: %s (error_code=%d, error_domain=%s)",
               __func__,
               (error != NULL) ? error->message : "N/A",
               (error != NULL) ? error->code : -1,
               (error != NULL) ? g_quark_to_string(error->domain) : "N/A");
    ...
    syslog(LOG_INFO, "%s: VDO stream started successfully", __func__);
